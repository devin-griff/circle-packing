...
Replace `circle-packing`, `Circle Packing`, `griffith-pse-circle-packing`, and `Non-convex NLP for the smallest enclosing rectangle` in
every text file (including the Dockerfile and fly.toml):

```bash
find . -type f \( -name '*.py' -o -name '*.md' -o -name '*.toml' -o -name 'Dockerfile' \) \
    -exec sed -i \
    "s|circle-packing|$APP_SLUG|g; \
     s|Circle Packing|$APP_TITLE|g; \
     s|griffith-pse-circle-packing|griffith-pse-$APP_SLUG|g; \
     s|Non-convex NLP for the smallest enclosing rectangle|$APP_TAGLINE|g" {} +
```
...
- Type **A**, name `circle-packing`, value `66.241.124.X` (Fly's edge — get the
  exact IP from `flyctl certs add` below; often the same IP used by other
  apps in your org)
- Type **AAAA**, name `circle-packing`, value `2a09:8280:1::112:XXXX:0`
...
[Launch demo →](https://circle-packing.griffith-pse.com){.btn .btn-primary target="_blank"}
:::
```
